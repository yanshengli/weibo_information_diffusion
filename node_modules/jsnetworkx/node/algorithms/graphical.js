...
function isValidDegreeSequenceHavelHakimi(degreeSequence) {
  ...
  try {
    var _temp = basicGraphicalTests(degreeSequence);

    var _temp2 = _slicedToArray(_temp, 5);
...
function isValidDegreeSequenceErdosGallai(degreeSequence) {
  ...
  try {
    var _temp3 = basicGraphicalTests(degreeSequence);

    var _temp32 = _slicedToArray(_temp3, 5);

    maxDegree = _temp32[0];
    minDegree = _temp32[1];
    _ = _temp32[2];
    n = _temp32[3];
    numDegrees = _temp32[4];
    _temp3;
  } catch (ex) {
    if (ex instanceof _exceptions.JSNetworkXUnfeasible) {
      return false;
